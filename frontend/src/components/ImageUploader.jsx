...
const ImageUploader = ({ onUploadSuccess, initialImageUrl = null }) => {
  ...
  return (
    <Box>
      ...
      {/* Zona de Carga */}
      <Box sx={uploaderStyles} onClick={triggerFileInput} role="button" tabIndex={0} aria-label="Seleccionar imagen para estampar">
        <UploadIcon size={48} color="var(--gray-700)" style={{ marginBottom: 'var(--spacing-xs)' }}/>
        <Typography variant="body1" color="var(--gray-900)">
          {selectedFile ? `Archivo: ${selectedFile.name}` : (uploadedUrl ? 'Imagen Cargada' : 'Selecciona o arrastra tu imagen')}
        </Typography>
        <Typography variant="caption" color="var(--gray-700)">
          (JPG, PNG, GIF, WEBP, SVG - Máx 10MB)
        </Typography>
         {/* Indicador de Estado */}
         {uploading && <CircularProgress size={20} sx={{ display: 'block', margin: 'var(--spacing-xs) auto 0' }} />}
         {!uploading && uploadedUrl && <CheckCircle size={20} color="var(--success)" style={statusIconStyles} />}
      ...
      {previewUrl && (
        <Box textAlign="center" mb={1}>
          <Typography variant="caption" color="textSecondary">Previsualización:</Typography>
