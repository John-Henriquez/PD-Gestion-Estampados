...
export async function createOrder(orderData) {
  try {
    ...
    throw error.response?.data || new Error(error.message || 'Error de red o del servidor al crear el pedido');
  }
}
...
export async function getMyOrders() {
    try {
        ...
        throw error.response?.data || new Error(error.message || 'Error de red o del servidor al obtener mis pedidos');
    }
}
...
export async function getAllOrders() {
    try {
        ...
        console.error('Error al obtener todos los pedidos (Admin):', error.response?.data || error.message);
...
export async function updateOrderStatus(orderId, newStatus) {
    try {
        ...
        if (response.status === 200 && response.data.status === 'Success') {
            ...
            throw new Error(response.data.message || 'Error inesperado al actualizar el estado del pedido');
        }
    } catch (error) {
        console.error(`Error al actualizar estado del pedido ${orderId}:`, error.response?.data || error.message);
        throw error.response?.data || new Error(error.message || 'Error de red o del servidor');
    }
}