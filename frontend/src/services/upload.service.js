...
export async function uploadImage(file) {
  ...
  try {
    ...
    throw error.response?.data || new Error(error.message || 'Error de red o del servidor al subir imagen.');
  }
}
...
export async function uploadStampImage(file) {
  ...
  try {
    ...
    if (response.status === 200 && response.data.status === 'Success' && response.data.data?.imageUrl) {
      console.log('Imagen de estampado subida:', response.data.data.imageUrl);
      return response.data.data.imageUrl; 
    } else {
      throw new Error(response.data.message || 'Respuesta inesperada del servidor al subir imagen de estampado.');
    }
  } catch (error) {
